...
public class AICDummy {
	public static void main(String[] args) throws IOException {
		ArrayList<AICinfo> list = new ArrayList<AICinfo>();//중복검사용
		...
		//색상,형태 등..
		String[] color = {"화이트"};
		String[] door = {"벽걸이","스탠드","창문형"};
		
		//더미데이터 파일 불러오기
		BufferedWriter writer = new BufferedWriter(new FileWriter("DummyData\\AICinfo.txt"));
		BufferedWriter writer2 = new BufferedWriter(new FileWriter("DummyData\\AICmanage.txt"));
		
		//50개 생성
		for (int i=1; i<=50; i++) {
			//관리번호
			String itemCode = "AIC_20230815_" + i;
			//카테고리
			String category = "에어컨";
			//기업명
			...
			//L
			String volume = (rnd.nextInt(10)+8)+"평";
			//제품명
			String code = "AIC";
			//중간 숫자 5개 생성
			for (int j = 0; j < 5; j++) {
			...
			//제품명
			String name = ent + " " +names[rnd.nextInt(names.length)]+category+" "+doorChoice;
			//---------------------------------
			...
			AICinfo ri = new AICinfo(itemCode, category, ent, name, code, colorChoice, doorChoice, volume);
			list.add(ri);
			//추가
